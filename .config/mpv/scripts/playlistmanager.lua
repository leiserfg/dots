...
local settings = {
  ...
  --path where you want to save playlists
  playlist_savepath = "~",
...
local user_home  = (
  os.getenv('HOME') -- Unix and Windows 
  or os.getenv('USERPROFILE') -- Sometimes on Windows
  or (os.getenv('HOMEDRIVE') .. '\\' .. os.getenv('HOMEPATH')) -- Other times on Windows 
)  

function expand(path)
  if path:find('~') == 1 then
    return path:gsub('~', user_home, 1)
  else
    return path
  end
end
...
function save_playlist()
  ...
  local playlist_savepath = expand(settings.playlist_savepath)
  local savepath = utils.join_path(playlist_savepath, os.date().."-size_"..length.."-playlist.m3u")
